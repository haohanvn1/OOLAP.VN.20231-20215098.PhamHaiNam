...
public class Cart {
    ...
    public void addMedia(Media media) throws LimitExceededException, DuplicatedItemException {
        if (itemsOrdered.size() == MAX_NUMBERS_ORDERED) {
            throw new LimitExceededException("ERROR: Number of items has reached the limit.");
        }

        if (itemsOrdered.contains(media)) {
            throw new DuplicatedItemException("ERROR: Item is already in the cart.");
        }
    ...
    public void removeMedia(Media media) {
        if (itemsOrdered.remove(media)) {
            System.out.println("Removed " + media.toString() + " from the cart.");
        } else {
    ...
    public void searchByTitle(String title) {
        boolean found = false;
        System.out.println("Search result for keyword: " + title);
        for (Media m : itemsOrdered) {
            if (m.isMatch(title)) {
                System.out.println(m.toString());
                found = true;
